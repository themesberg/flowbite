...
If you are installing Phoenix version 1.7 or later you can skip this part and go directly to the Flowbite installation guide because Tailwind CSS is automatically configured.
...
1. Install the Flowbite package using NPM:
...
1. Now we need to find a [hero section](https://flowbite.com/blocks/marketing/hero/) from the Flowbite Blocks collection and add it inside the `index.html.heex` file:
...
Remove the default `container` class from the `app.html.heex` file to enable the built-in container from Flowbite Blocks.

To finish the page you can check out the Marketing UI website sections from the [Flowbite Blocks](https://flowbite.com/blocks/) collection to add new feature, content, contact, FAQ, pricing, and footer sections to this current and future pages.
