...
const Default = {
    ...
    containerClasses:
        'absolute bg-white border mt-2 border-gray-200 rounded-lg z-[60] text-gray-500 text-sm py-3 w-full hidden',
    menuClasses: 'font-medium text-gray-500',
    menuItemClasses:
        'flex items-center py-2 px-5 cursor-pointer hover:bg-gray-100',
    onHide: () => {},
};
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('[data-autocomplete]').forEach((inputEl) => {
        const menuEl = document.getElementById(
            inputEl.getAttribute('data-autocomplete')
        );
