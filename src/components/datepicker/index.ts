...
export function initDatepickers($rootElement: RootElement = document) {
    $rootElement
        .querySelectorAll(
            '[datepicker], [inline-datepicker], [date-rangepicker]'
        )
        .forEach(initDatepickerByElement);
}

function initDatepickerByElement($datepickerEl: Element) {
    if ($datepickerEl) {
        ...
        const rangePicker = $datepickerEl.hasAttribute('date-rangepicker');
        new Datepicker(
...
if (typeof window !== 'undefined') {
    ...
    window.initDatepickerByElement = initDatepickerByElement;
}
